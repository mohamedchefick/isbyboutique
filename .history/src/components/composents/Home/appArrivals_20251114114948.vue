<template>
    <div class="arrivals-container mx-5 my-10" ref="arrivalsContainer">
        <h2 class="text-center text-3xl font-bold mb-8">{{ $t ('new') }}</h2>
        ...
        <div class="bg-white p-6 rounded-lg shadow">
           <div v-if="products.length" class="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <p v-else class="text-gray-500 text-center py-6">Aucun produit disponible</p>
        </div>

        <div v-if="selectedProduct" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" @click.self="selectedProduct = null">
...
onMounted(() => {
    ...
    onSnapshot(q, (snapshot) => {
        ...
        // Délai pour que le DOM se mette à jour avant animation
