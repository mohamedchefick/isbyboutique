...
async function saveClientInfo() {
  ...
  const cartItemsWithImages = storedCart.map(item => {
    console.log("📦 Article brut:", item); // DEBUG - voir la structure de chaque article
    
    const imageUrl = item.image || item.img || item.imageUrl || item.thumbnail || item.photo || '';
    console.log("🖼️ Image URL trouvée:", imageUrl); // DEBUG
